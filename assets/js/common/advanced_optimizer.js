...
window.render3DCore = async function (coreDataString) {
    ...
    // --- 1. DÜZELTME: Geometri ve Pencere Yüksekliği Doğrulaması (Taşmanın Asıl Nedeni) ---
    if (E >= A) E = A * 0.8;
    if (D >= E) D = E * 0.5;

    let legHeight = F > 0 ? F : (B - D / 2);

    // Eğer veritabanındaki F değeri EP nüvelerde "yarı pencere" yerine "tam pencere" girilmişse:
    if (legHeight >= B) {
        legHeight = (F / 2 > 0) ? (F / 2) : (B * 0.8);
    }
    // Negatif arka plaka (ters çizim) oluşmasını kesin olarak engelleyen güvenlik sınırı:
    if (legHeight >= B) {
        legHeight = B * 0.8;
    }
    ...
    function createCoreHalf(isTop) {
        ...
        let centerLegGeom;
        // --- 2. DÜZELTME: EP nüvelerin orta bacağının yuvarlak çizilmesi ("EP" eklendi) ---
    ...
    function createWinding(radiusIn, radiusOut, material, turns, fillFactor = 0.6) {
        ...
        // --- 3. DÜZELTME: Y ekseni taşmasını engelleme ---
        ...
        for (let l = 0; l < renderLayers; l++) {
            ...
            // --- 4. DÜZELTME: Y Ekseni tam merkezleme ---
    ...
    if (family === "RM" || family === "PQ" || family === "PM") {
        coreMaterialVolMm3 += 2 * (Math.PI * Math.pow(A / 2, 2) * backPlateThick);
    } else {
    ...
    // Side legs: In the RM/PQ/PM (pot-core) family, the actual outer shell is a seamless 360°
    // circular shell(cup shape), unlike the simplified
    // "2 separate rectangular legs" model in 3D rendering.
    // The box approach significantly underestimates the weight because it does not account for
    // a large part of this shell(the solid ferrite area on the sides).
...
window.runCustomThermalTest = function () {
    ...
    if (swEl && res.switches && res.switches.length > 0) {
        ...
        // Conduction loss: always /qty (physically precise) since the current is shared among qty units. 
        // Switching loss: if the drive is powered according to the number of parallel units (gateScales=true), 
        // the switching time remains ~constant even though the total gate load is met -> p_sw_W is taken as constant.
        // If the drive is fixed (gateScales=false, default/safe side), the switching time and therefore the loss also 
        // increase approximately proportionally with qty, as the total gate capacitance increases with qty.
    ...
    const genCoreTable = (title, data) => {
        ...
        data.forEach((item, index) => {
            ...
            if (!item.totalCost || item.totalCost === 999 || item.costPerUnit === 999) {
                ...
                const setLabel = item.isTwoPieceSet ? "(2-piece set)" : "(2-core set)";
                costDisplay = `<b>${sanitizeHTML(item.totalCost.toFixed(2))} $</b> <br>` +
                    `<span style="font-size:10px;color:#aaa;">${setLabel} | Tek: ${sanitizeHTML(item.costPerUnit.toFixed(2))} $</span>`;
            }
