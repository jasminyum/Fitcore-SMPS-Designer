...
window.openAdvancedTable = function () {
    ...
    let autoRth = estPloss > 0 ? (40 / estPloss) : 2.0;
    autoRth = Math.max(0.1, Math.min(autoRth, 50)).toFixed(1);
        ...
        <div class="adv-box" style="box-shadow:0 3px 6px rgba(0,0,0,0.5);">
            ...
            <div class="d-flex flex-wrap align-items-center gap-3">
                ...
                <div class="d-flex flex-wrap align-items-center gap-2 mt-2 mt-lg-0 w-100">
                    ...
                    <div class="input-group input-group-sm" style="width: auto;" title="${sanitizeHTML(safeGetT('adv_cooling_tooltip') || 'Termal analiz ve soğutma profili seçiminiz.')}">
                        <span class="input-group-text bg-dark text-light border-secondary">${sanitizeHTML(safeGetT('adv_cooling_label') || 'Soğutma Tipi')}</span>
                        <select id="advRthSelect" class="form-select bg-dark text-light border-secondary" style="max-width: 170px;" onchange="document.getElementById('manualRthWrapper').style.display = (this.value === 'manual') ? 'flex' : 'none'">
                            ...
                            <option value="40.0">${sanitizeHTML(safeGetT('adv_cool_closed') || 'Doğal Konveksiyon (Kapalı)')}</option>
                            <option value="25.0">${sanitizeHTML(safeGetT('adv_cool_open') || 'Doğal Konveksiyon (Açık)')}</option>
                            <option value="12.0">${sanitizeHTML(safeGetT('adv_cool_small_hs') || 'Küçük Soğutucu')}</option>
                            <option value="6.0">${sanitizeHTML(safeGetT('adv_cool_large_hs') || 'Büyük Soğutucu')}</option>
                            <option value="2.5">${sanitizeHTML(safeGetT('adv_cool_fan') || 'Fan Soğutma (Zorlanmış)')}</option>
                            <option value="0.5">${sanitizeHTML(safeGetT('adv_cool_water') || 'Özel / Sıvı')}</option>
                        </select>
                    </div>
...
window.render3DCore = async function (coreDataString) {
    try {
        await window.loadThreeJS(); // 3D Motorunun yüklenmesini garanti altına alıyoruz
    } catch (e) {
    ...
    function createWinding(radiusIn, radiusOut, material, turns, fillFactor = 0.6) {
        ...
        let layers = Math.floor(availableThick / layerPitch);
        let turnsPerLayer = Math.floor(availableHeight / turnPitch);
        ...
        if (renderLayers * renderTurnsPerLayer > MAX_TORUS) {
            ...
            renderWireD = availableHeight / renderTurnsPerLayer;
        }
        ...
        for (let l = 0; l < renderLayers; l++) {
            ...
            const currentLayerTurns = isOddLayer ? renderTurnsPerLayer - 1 : renderTurnsPerLayer;
            const startY = -((currentLayerTurns * turnPitch) / 2) + (renderWireD / 2) + yOffsetHex;
            ...
            for (let t = 0; t < currentLayerTurns; t++) {
                ...
                torus.position.y = startY + (t * turnPitch);
                group.add(torus);
            }
        }
        return group;
    ...
    const coreWeightGrams = (coreData.volume || 0) * 4.8;
...
window.executeAdvancedOptimization = async function () {
    ...
    try {
        ...
        const maxStrandD = 2 * (delta_m * 1000);
...
window.filterResultsByManufacturer = function () {
    ...
    const rho = 0.01724 * (1 + 0.00393 * (currentTemp - 20));
    ...
    if (currents.isDualCoil) {
        ...
        if (bestCoil1 && wire1) {
            ...
            let dcr = rho * ((n1 * MLT) / 1000) / parseFloat(wire1.totalArea);
        ...
        if (bestCoil2 && wire2) {
            ...
            let dcr = rho * ((n1 * MLT) / 1000) / parseFloat(wire2.totalArea);
...
function renderAdvancedResults(res, skinDepthD, states) {
    ...
    const genCoreTable = (title, data) => {
        ...
        data.forEach((item, index) => {
            const unitPriceDouble = item.costPerUnit * 2;
            const costDisplay = item.totalCost === 999
                ? `<span style='color:#9e9e9e'>${sanitizeHTML(safeGetT('adv_no_price') || 'Fiyat Yok')}</span>`
                : `${sanitizeHTML(unitPriceDouble.toFixed(2))} (2-core set / excl. coil)`;
    ...
    const genWireTable = (title, data) => {
        ...
        data.forEach((wire, index) => {
            ...
            const safeCoating = sanitizeHTML(wire.coating);
...
window.exportAdvancedResultsToCSV = function () {
    ...
    const addCoreToCSV = (title, data) => {
        ...
        csvContent += `\n--- ${title} ---\n`;
        csvContent += "ID,Core_Name,Material,Bobbin,Bmax_mT,Pv(mW/cm3),Core_Loss(W),Manufacturer,Unit_Price,Unit,Total_Price,Fuzzy_Score\n";
        data.forEach((row, index) => {
            const costStr = row.totalCost === 999 ? (safeGetT('adv_unknown') || 'Bilinmiyor') : (row.costPerUnit * 2).toFixed(2);
            const totCostStr = row.totalCost === 999 ? (safeGetT('adv_unknown') || 'Bilinmiyor') : (row.totalCost * 2).toFixed(2);
            csvContent += `${index + 1},${csvEscape(row.name)},${csvEscape(row.material)},` +
                ...
                `${csvEscape(row.distributor)},${costStr},${row.stackCount},${totCostStr},${row.fuzzyScore.toFixed(2)}\n`;
        });
    };
...
window.runCustomThermalTest = function () {
    ...
    if (swEl && res.switches && res.switches.length > 0) {
        ...
        selSwitchName = `${sw.name} (x${qty})`;
        switchLoss = (sw.p_cond_W / qty) + (sw.p_sw_W * qty);
    } else {
