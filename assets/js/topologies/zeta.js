// ================================================================
// Zeta Converter Calc (Modern Architecture)
// ================================================================
...
function toggleEffMode() {
    ...
    if (effMode === "ideal") {
        ...
        if (rectMode === "async") {
            ...
            if (lblVd) lblVd.innerText = (typeof getT === "function") ? getT("lbl_diode_vd") : "Diode Vd [V]";
            if (lblTrr) lblTrr.innerText = (typeof getT === "function") ? getT("lbl_trr_irr") : "trr [ns] / Irr [A]";
            if (thPonH) thPonH.innerText = (typeof getT === "function") ? getT("th_diode_cond") : "Diyot Ýletim (Pdiode_cond)";
            if (thPdiode) thPdiode.innerText = (typeof getT === "function") ? getT("th_diode_rr") : "Diyot Rev. Rec. (Prr)";
        } else {
            ...
            if (lblVd) lblVd.innerText = (typeof getT === "function") ? getT("lbl_body_diode_vsd") : "Body Diode Vsd [V]";
            if (lblTrr) lblTrr.innerText = (typeof getT === "function") ? getT("lbl_body_diode_trr") : "Body Diode trr [ns] / Irr [A]";
            if (thPonH) thPonH.innerText = (typeof getT === "function") ? getT("th_high_side_cond") : "Senkron Ýletim (Pon_H)";
            if (thPdiode) thPdiode.innerText = (typeof getT === "function") ? getT("th_ext_diode_sync") : "Gövde Diyot (Prr)";
        }
    }
}
...
// ================================================================
// GÝRÝÞ DOÐRULAMA (Zeta Pozitif Çýkýþ Zorlamasý)
// ================================================================
function checkUserInput() {
    ...
    if (vout < 0) {
        var zetaModalEl = document.getElementById('zetaWarningModal');
        if (zetaModalEl) {
            var zetaModal = new bootstrap.Modal(zetaModalEl);
            zetaModal.show();
        }
        return false;
    }
    ...
    if (f_khz < 0.1 || f_khz > 2000) {
        alert((typeof getT === "function") ? getT("alert_freq_invalid") : "Frekans geçerli aralýkta olmalýdýr.");
        f_khz = 100.0;
    }
...
function updateChartsAndTable() {
    ...
    var d_max = (vout + Uf) / (vin_min + vout + Uf);
    var d_nom = (vout + Uf) / (vin_nom + vout + Uf);
    ...
    if (mode !== actualMode) {
        ...
        var modeNameTranslated = (typeof getT === "function") ? getT("mode_" + actualMode) : actualMode;
        var prefix = (typeof getT === "function") ? getT("warn_mode_prefix") : "Sistem ";
        var mid = (typeof getT === "function") ? getT("warn_mode_mid") : "V giriþinde ";
        var suffix = (typeof getT === "function") ? getT("warn_mode_suffix") : " modunda çalýþýyor.";
        modeWarnEl.innerHTML = prefix + vin_nom + mid + modeNameTranslated + suffix;
    } else {
    ...
    var Ccp_uF = (ilout * d_nom) / (0.05 * vout * f) * 1e6;
    var Cout_uF = window.deltaIL_L2 / (8 * f * (0.01 * vout)) * 1e6;
    ...
    if (effMode === "ideal") {
        ...
        var realRes = calculateRealEfficiency(vin_nom, vout, ilout, f, L1_H, L2_H, params, verim);
        finalVerim = realRes.efficiencyPercent;
        effData = generateRealEffCurve(vin_nom, vout, ilout, f, L1_H, L2_H, params, verim);
    ...
    var wf = generateAllWaveforms(vin_nom, vout, ilout, Iin_nom, t1_s, L1_H, L2_H, window.deltaIL_L1, window.deltaIL_L2, actualMode);
    drawCharts(wf, Iin_nom, ilout, vin_nom, vout, effData);
    updateResultTable(wf);
}

// ================================================================
// GERÇEK KAYIP HESAPLAMALARI (Zeta Converter)
// ================================================================
...
function calculateRealEfficiency(vin, vout, iout, f_sw_hz, L1_H, L2_H, p, eff_guess) {
    var rectMode = document.getElementById("rectifierMode") ? document.getElementById("rectifierMode").value : "async";
    ...
    var Iin_avg, Ic1_rms, Icout_rms, I_D1_avg;

    Iin_avg = (vout * iout / eff_guess) / vin;
...
function generateRealEffCurve(vin, vout, max_iout, f_hz, L1_H, L2_H, params, verim) {
    var values = [], labels = [];
    for (var pct = 10; pct <= 120; pct += 5) {
        var currentLoad = max_iout * (pct / 100);
        if (currentLoad <= 0.001) currentLoad = 0.001;

        var res = calculateRealEfficiency(vin, vout, currentLoad, f_hz, L1_H, L2_H, params, verim);
...
function drawCharts(wf, Iin, Iout, Vin, Vout_mag, effData) {
    ...
    function baseOpts(yTitle) {
        return {
            ...
            scales: {
                x: { type: 'category', ticks: { color: textColor, maxTicksLimit: 9, callback: function (val, idx) { return (idx % tickStep === 0) ? wf.labels[idx] + "µs" : ''; } }, title: { display: true, text: getT('chart_time_us') || 'Time (µs)', color: textColor }, grid: { color: gridColor, borderColor: gridColor } },
...
// ================================================================
// CIRCUITJS (FALSTAD) ZETA CONVERTER ENTEGRASYONU
// ================================================================
function openFalstadZetaSimulation() {
    ...
    var r_load = parseFloat(document.getElementById('rOutput').innerText) || 6;

    document.getElementById("simulationContainer").style.display = "block";
    document.getElementById("liveDataBox").style.display = "block";
    document.getElementById("liveDataBox").innerHTML = "Fitcore SMPS Designer: Zeta Converter";
    ...
    var v_c1_initial = -vout;

    var v_pwm_on = 15.0;
    var v_pwm_off = -(vin_nom + vout);

    var v_amp = (v_pwm_on - v_pwm_off) / 2;
    var v_offset = (v_pwm_on + v_pwm_off) / 2;

    var vscale = Math.max(5, Math.ceil(vout / 5) * 5);
    var iscale = Math.max(0.5, Math.ceil(ilout / 2) * 0.5);

    var p_out = vout * ilout;
    var mosfet_beta;

    if (mode === "critical") {
        mosfet_beta = (p_out <= 100) ? "2" : "1";
    } else if (mode === "discontinuous") {
        mosfet_beta = (p_out <= 100) ? "0.8" : "0.4";
    } else {
        mosfet_beta = (p_out <= 100) ? "4" : "2";
    }

    var falstadTemplate = `
$ 1 {TIMESTEP} 100.0 50 5.0 50
f 144 240 144 160 8 1.5 {MOSFET_BETA}
g 64 320 64 336 0
v 64 320 64 160 0 0 40 {VIN} 0 0 0.5
r 560 160 560 320 0 {R_VAL}
c 464 160 464 320 0 {COUT_VAL} {VOUT}
c 208 160 336 160 0 {C1_VAL} {V_C1}
l 208 240 208 320 0 {L1_VAL} {I_IN}
l 336 160 464 160 0 {L2_VAL} {ILOUT}
d 336 320 336 160 1 {UF}
w 64 160 128 160 0
w 160 160 208 160 0
w 64 320 208 320 0
w 208 320 336 320 0
...
w 464 160 560 160 0
w 560 320 464 320 0
v 208 240 144 240 0 2 {FREQ} {V_AMP} {V_OFFSET} 0 {DUTY}
w 208 160 208 240 0
o 6 1 0 33 {VSCALE} {ISCALE} 0 -1
o 6 1 0 34 {VSCALE} {ISCALE} 0 -1
o 7 1 0 33 {VSCALE} {ISCALE} 1 -1
o 7 1 0 34 {VSCALE} {ISCALE} 1 -1
o 8 1 0 33 {VSCALE} {ISCALE} 3 -1
o 3 1 0 34 {VSCALE} {ISCALE} 2 -1
`.trim();
    ...
    var circuitString = falstadTemplate
        ...
        .replace('{MOSFET_BETA}', mosfet_beta)
