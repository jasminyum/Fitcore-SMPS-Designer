...
function drawCharts(wf, ilout, effData) {
            ...
            scales: {
                x: {
                    ...
                    ticks: {
                        color: textColor,
                        maxTicksLimit: 9,
                        callback: function (val, idx) { return (idx % tickStep === 0) ? wf.labels[idx] + "µs" : ''; }
                    },
                    title: { display: true, text: 'Time (µs)', color: textColor },
    ...
    function mk(id, datasets, yTitle, stepped) {
        ...
        canvas.chart = new Chart(canvas.getContext('2d'), {
            type: 'line',
            data: { labels: wf.labels, datasets: datasets },
            options: baseOpts(yTitle, stepped)
        });
    }

    mk('vinChart', [
        { label: 'V1', data: wf.v1, borderColor: 'rgba(100, 181, 246, 1)', borderWidth: 2, fill: false }
    ], 'Voltage (V)');

    mk('idChart', [
        { label: 'Iin', data: wf.iin, borderColor: 'rgba(239, 83, 80, 1)', borderWidth: 2, fill: false }
    ], 'Current (A)');
    ...
    if (effCanvas) {
        ...
        effCanvas.chart = new Chart(effCanvas.getContext('2d'), {
            ...
            options: {
                ...
                scales: {
                    x: {
                        title: { display: true, text: 'Load (%)', color: textColor },
                        ticks: { color: textColor },
                        grid: { color: gridColor, borderColor: gridColor }
                    },
                    y: {
...
function openFalstadTwoSimulation() {
    ...
    document.getElementById("liveDataBox").innerHTML = "Fitcore SMPS Designer: TWO TRANSISTOR FORWARD Converter";
    ...
    var falstad_loss_factor = 1.03;
    var duty_cycle = (duty_ideal + duty_drop) * falstad_loss_factor;
    ...
    var vscale_ds = Math.max(50, Math.ceil(vin_nom * 1.5 / 50) * 50);

    var falstadTemplate = `
$ 1 {TIMESTEP} 10.0 50 5.0 50
T 208 192 336 256 0 {L_PRI} {RATIO} {COUPLING} 48
f 192 320 240 320 32 1.5 0.025
d 240 272 240 160 1 {UF}
d 416 272 416 192 1 {UF}
l 416 192 528 192 0 {L_VAL} 0
c 528 192 528 272 0 {C_VAL} {V_INIT}
r 592 192 592 272 0 {R_VAL}
...
w 80 128 144 128 0
g 336 272 336 288 0
d 336 192 416 192 1 {UF}
f 96 176 144 176 32 1.5 0.025
d 144 384 144 240 1 {UF}
w 208 256 240 272 0
w 144 160 144 128 0
w 48 384 144 384 0
w 144 384 240 384 0
w 208 192 192 192 0
g 48 384 48 400 0
v 96 240 96 176 0 2 {FREQ} {V_AMP} {V_OFFSET} 0 {DUTY}
w 144 192 144 240 0
v 176 320 192 320 0 2 {FREQ} {V_AMP} {V_OFFSET} 0 {DUTY}
r 96 240 144 240 0 10
r 240 304 288 304 0 {R_SNUB}
c 288 336 288 304 0 {C_SNUB} 0
r 176 320 144 384 0 10
c 192 192 192 160 0 {C_SNUB} 0
r 144 160 192 160 0 {R_SNUB}
w 192 192 144 192 0
w 240 128 240 160 0
w 288 336 240 336 0
o 7 1 0 34 {VSCALE_IN} 0.05 0 -1
o 5 1 0 34 {VSCALE_OUT} 0.05 1 -1
o 6 1 0 33 {ISCALE_OUT} 0.1 2 -1
o 1 1 0 34 {VSCALE_DS} 0.05 3 -1
`.trim();
...
window.openSelectedTable = function () {
    ...
    // ---------------------------------------------------------------------------------
    // FIX: RMS current calculations are performed BEFORE Advanced Mode is invoked
    // ---------------------------------------------------------------------------------
...
document.getElementById('printButton').addEventListener('click', function () { printPage(); });
