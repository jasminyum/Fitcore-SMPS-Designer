// ================================================================
// SEPIC Converter Calc (Modern Architecture)
// ================================================================
...
function updateChartsAndTable() {
    ...
    var Cin_uF = window.deltaIL_L1 / (8 * f * (0.01 * vin_nom)) * 1e6;
    var Ccp_uF = (ilout * d_nom) / (0.05 * vin_min * f) * 1e6;
    var Cout_uF = (ilout * d_nom) / (0.01 * vout * f) * 1e6;
    ...
    if (effMode === "ideal") {
        ...
        var realRes = calculateRealEfficiency(vin_nom, vout, ilout, f, L1_H, L2_H, params);
        finalVerim = realRes.efficiencyPercent;
        effData = generateRealEffCurve(vin_nom, vout, ilout, f, L1_H, L2_H, params);
    ...
    document.getElementById('cinOutput').innerText = Cin_uF.toFixed(2);
...
// ================================================================
// GERÇEK KAYIP HESAPLAMALARI (SEPIC)
// ================================================================
...
function calculateRealEfficiency(vin, vout, iout, f_sw_hz, L1_H, L2_H, p) {
    ...
    var Iin_avg, Icin_rms, Ic1_rms, Icout_rms;
    ...
    if (K < Kcrit) {
        ...
        Icin_rms = Math.sqrt(Math.max(0, Math.pow(I_L1_rms, 2) - Math.pow(Iin_avg, 2)));
        Ic1_rms = Math.sqrt(D * Math.pow(IL2_pk, 2) / 3 + D2 * Math.pow(IL1_pk, 2) / 3);
        Icout_rms = Math.sqrt(Math.max(0, Math.pow(I_D1_rms, 2) - Math.pow(iout, 2)));
        ...
        Icin_rms = dIL1 / Math.sqrt(12);
        Ic1_rms = iout * Math.sqrt(D / (1 - D));
        Icout_rms = Math.sqrt(Math.max(0, Math.pow(I_D1_rms, 2) - Math.pow(iout, 2)));
    }

    // ==============================================================
    // ORTAK KAYIP HESAPLAMALARI
    // ==============================================================
    ...
    var Ptotal = Pon_H + Pdiode_cond + Pon_L + Psw_L + Prr + Pcoss + Pd + Pg + Pic + Pl1_dcr + Pl2_dcr + Pcin + Pc1 + Pcout;
...
function drawCharts(wf, Iin, Iout, Vin, Vout, effData) {
...
// ================================================================
// CIRCUITJS (FALSTAD) SEPIC CONVERTER ENTEGRASYONU
// ================================================================
function openFalstadSepicSimulation() {
    ...
    var cin_uF = parseFloat(document.getElementById('cinOutput').innerText) || 10;
    ...
    document.getElementById("liveDataBox").innerHTML = "Fitcore SMPS Designer: SEPIC Converter";
    ...
    // --- DCM/CCM Kontrolü ve Dinamik Duty Cycle Hesaplamasý ---
    var d_nom_ccm = (vout + Uf) / (vin_nom + vout + Uf);
...
v -16 224 -16 80 0 0 40 {VIN} 0 0 0.5
v 96 160 160 160 0 2 {FREQ} {V_AMP} {V_AMP} 0 {DUTY}
g 208 224 208 240 0
d 304 80 384 80 1 {UF}
w 208 80 208 144 0
w 208 176 208 224 0
f 160 160 208 160 32 1.5 1000
r 448 80 448 224 0 {R_VAL}
c 384 80 384 224 0 {COUT_VAL} {VOUT}
l 304 224 304 80 0 {L2_VAL} {ILOUT}
c 208 80 304 80 0 {C1_VAL} {VIN}
l 32 80 208 80 0 {L1_VAL} {I_IN}
c 32 80 32 224 0 {CIN_VAL} {VIN}
w -16 80 32 80 0
w -16 224 32 224 0
w 32 224 208 224 0
w 208 224 304 224 0
...
w 384 224 448 224 0
w 384 80 448 80 0
o 11 1 0 33 {VSCALE} {ISCALE} 0 -1
o 9 1 0 33 {VSCALE} {ISCALE} 1 -1
o 4 1 0 34 {VSCALE} {ISCALE} 2 -1
o 3 1 0 33 {VSCALE} {ISCALE} 3 -1
o 7 1 0 34 {VSCALE} {ISCALE} 4 -1
`.trim();
